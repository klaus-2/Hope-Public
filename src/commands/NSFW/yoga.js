...
module.exports = class Yoga extends Command {
    constructor(bot) {
        super(bot, {
            name: 'yoga',
            nsfw: true,
    ...
    async run(bot, message, settings) {
        ...
        fetch(`https://www.reddit.com/r/${randSubreddit}.json`).then(res => res.json()).then(data => {
            ...
            return new Promise(async (resolve, reject) => {
                ...
                /*var hook = await message.channel.createWebhook(WebhookNames[random], { avatar: `https://i.imgur.com/${WebhookAvatars[random]}.gif`, reason: `command ${settings.prefix}yoga by ${message.author.username}` })
    ...
    async callback(bot, interaction, guild) {
        ...
        fetch(`https://www.reddit.com/r/${randSubreddit}.json`).then(res => res.json()).then(data => {
            ...
            if (!reddit) return interaction.reply({ embeds: [channel.error('ERROR_MESSAGE', `Falha em r/${randSubreddit}`, true)], ephemeral: true });
