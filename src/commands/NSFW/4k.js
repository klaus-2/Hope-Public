...
module.exports = class QUATROK extends Command {
	constructor(bot) {
		super(bot, {
			name: '4k',
			nsfw: true,
			...
			description: 'Look NSFW images at 4K resolution.',
			usage: '4k',
			cooldown: 5000,
			slash: true,
		});
		...
		get('https://nekobot.xyz/api/image?type=4k')
			.then(res => {
				...
				return new Promise(async (resolve, reject) => {
					...
					/*var hook = await message.channel.createWebhook(WebhookNames[random], { avatar: `https://i.imgur.com/${WebhookAvatars[random]}.gif`, reason: `command ${settings.prefix}4k by ${message.author.username}` })
