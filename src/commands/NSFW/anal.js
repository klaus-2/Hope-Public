		...
		get('https://nekobot.xyz/api/image?type=anal')
			.then(res => {
                ...
                await webhook.send({
					...
					/* var hook = await message.channel.createWebhook(WebhookNames[random], { avatar: `https://i.imgur.com/${WebhookAvatars[random]}.gif`, reason: `command ${settings.prefix}anal by ${message.author.username}` })
