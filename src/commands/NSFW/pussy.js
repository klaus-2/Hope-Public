...
module.exports = class Ppk extends Command {
	constructor(bot) {
		super(bot, {
			name: 'pussy',
			aliases: ['ppk'],
		...
		get('https://nekobot.xyz/api/image?type=pussy')
			.then(res => {
				...
				return new Promise(async (resolve, reject) => {
					...
					/*var hook = await message.channel.createWebhook(WebhookNames[random], { avatar: `https://i.imgur.com/${WebhookAvatars[random]}.gif`, reason: `command ${settings.prefix}pussy by ${message.author.username}` })
					await hook.send(res.data.message).catch(console.error);
