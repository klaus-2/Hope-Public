...
module.exports = class Peitos extends Command {
	constructor(bot) {
		super(bot, {
			name: 'boobs',
			aliases: ['peitos'],
		...
		get('https://nekobot.xyz/api/image?type=boobs')
			.then(res => {
				...
				return new Promise(async (resolve, reject) => {
					...
					/*var hook = await message.channel.createWebhook(WebhookNames[random], { avatar: `https://i.imgur.com/${WebhookAvatars[random]}.gif`, reason: `command ${settings.prefix}boobs by ${message.author.username}` })
