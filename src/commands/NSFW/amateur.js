...
module.exports = class Amador extends Command {
    constructor(bot) {
        super(bot, {
            name: 'amateur',
            aliases: ['amador'],
            ...
            description: 'Look at amateur NSFW images.',
            usage: 'amateur',
            cooldown: 5000,
            slash: true,
        });
    ...
    async run(bot, message, settings) {
        ...
        fetch(`https://www.reddit.com/r/${randSubreddit}.json`).then(res => res.json()).then(data => {
            ...
            return new Promise(async (resolve, reject) => {
                ...
                /*var hook = await message.channel.createWebhook(WebhookNames[random], { avatar: `https://i.imgur.com/${WebhookAvatars[random]}.gif`, reason: `command ${settings.prefix}amateur by ${message.author.username}` })
