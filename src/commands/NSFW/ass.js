...
module.exports = class Bunda extends Command {
	constructor(bot) {
		super(bot, {
			name: 'ass',
			aliases: ['bunda'],
			...
			description: 'Look at ass NSFW images.',
			usage: 'bunda',
			cooldown: 5000,
			slash: true,
		});
		...
		get('https://nekobot.xyz/api/image?type=ass')
			.then(res => {
				...
				return new Promise(async (resolve, reject) => {
					...
					/*var hook = await message.channel.createWebhook(WebhookNames[random], { avatar: `https://i.imgur.com/${WebhookAvatars[random]}.gif`, reason: `command ${settings.prefix}ass by ${message.author.username}` })
