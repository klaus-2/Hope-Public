...
module.exports = class TFT extends Command {
  constructor(bot) {
    super(bot, {
      name: 'tft',
      dirname: __dirname,
      botPermissions: [ Flags.SendMessages, Flags.EmbedLinks],
      description: 'Displays a user\'s elo on the TFT.',
  ...
  async run(bot, message, settings) {
    ...
    const url2 = `https://${região}.api.riotgames.com/tft/league/v1/entries/by-summoner/`;
        ...
        res.on('end', () => {
            ...
            https.get((url2 + body.id + api_link), res => {
                ...
                res.on('end', () => {
                    ...
                    if (body.name == null) {
                        return message.channel.error('Foi mal, mas esse invocador não existe.\nVerifique se escreveu o nome corretamente e tente outra vez.').then(m => m.timedDelete({ timeout: 10000 }));
                        ...
                        const exampleEmbed = new Embed(bot, message.guild)
                            ...
                            .addFields(
                                //{ name: '\u200B', value: '\u200B' },
                                { name: message.translate('Games/elo:ELO'), value: `${body.summonerLevel}`, inline: false },
                                { name: 'Solo/Duo', value: 'Unranked', inline: true },
                                { name: 'Flex', value: 'Unranked', inline: true },
                                { name: message.translate('Games/elo:ELO2'), value: `0/0`, inline: true },
                                { name: message.translate('Games/elo:ELO3'), value: `-/-`, inline: true },
                                { name: message.translate('Games/elo:ELO4'), value: `0`, inline: true }
                            )
                        ...
                        const attachment = new AttachmentBuilder(`${process.cwd()}/assets/elos/Emblem_${body2[1].tier}.png`, { name: 'Hope-Elo.png' });

                        const exampleEmbed = new Embed(bot, message.guild)
                            .setColor(cor[body2[1].tier])
                            .setTitle(message.translate('Games/elo:ELO15'))
                            .setAuthor({ name: `${body.name}`, iconURL: 'http://ddragon.leagueoflegends.com/cdn/11.13.1/img/profileicon/' + body.profileIconId + '.png' })
                            ...
                            .addFields(
                                { name: message.translate('Games/elo:ELO'), value: `${body.summonerLevel}`, inline: false },
                                { name: 'Solo/Duo', value: flex_rank, inline: true },
                                { name: 'Flex', value: solo_duo_rank, inline: true },
                                { name: message.translate('Games/elo:ELO2'), value: `${body2[1].wins}/${body2[1].losses}`, inline: true },
                                { name: message.translate('Games/elo:ELO3'), value: `${Math.round(body2[1].wins / (body2[1].wins + body2[1].losses) * 100)}%`, inline: true },
                                { name: message.translate('Games/elo:ELO4'), value: `${body2[1].leaguePoints}`, inline: true }
                            )
                            //.setDescription(`**${body2[0].queueType}**\nLevel: ${data.summonerLevel}\nElo: ${body2[0].tier} ${body2[0].rank}\nV/D: ${body2[0].wins}/${body2[0].losses}\nWin Rate: ${Math.round(body2[0].wins/(body2[0].wins + body2[0].losses) * 100)}%\nPDL: ${body2[0].leaguePoints}`)
                            .setFooter({ text: `${message.translate('misc:FOOTER_GLOBALL', { username: message.author.username })} ${message.translate('misc:FOOTER_GLOBAL', { prefix: settings.prefix })}${message.translate(`Games/${this.help.name}:USAGEE`)}`, iconURL: `${message.author.displayAvatarURL({ format: 'png', dynamic: true })}` })
                            .setTimestamp()
                        ...
                        const exampleEmbed = new Embed(bot, message.guild)
                            ...
                            .addFields(
                                ...
                                { name: 'Flex', value: 'Unranked', inline: true },
                                { name:  message.translate('Games/elo:ELO2'), value: `${body2[0].wins}/${body2[0].losses}`, inline: true },
                                { name: message.translate('Games/elo:ELO3'), value: `${Math.round(body2[0].wins / (body2[0].wins + body2[0].losses) * 100)}%`, inline: true },
                                { name: message.translate('Games/elo:ELO4'), value: `${body2[0].leaguePoints}`, inline: true }
                            )
                            //.setDescription(`**${body2[0].queueType}**\nLevel: ${data.summonerLevel}\nElo: ${body2[0].tier} ${body2[0].rank}\nV/D: ${body2[0].wins}/${body2[0].losses}\nWin Rate: ${Math.round(body2[0].wins/(body2[0].wins + body2[0].losses) * 100)}%\nPDL: ${body2[0].leaguePoints}`)
                            .setFooter({ text: `${message.translate('misc:FOOTER_GLOBALL', { username: message.author.username })} ${message.translate('misc:FOOTER_GLOBAL', { prefix: settings.prefix })}${message.translate(`Games/${this.help.name}:USAGEE`)}`, iconURL: `${message.author.displayAvatarURL({ format: 'png', dynamic: true })}` })
                            .setTimestamp()
