...
const { Embed } = require(`../../utils`),
    ...
    baseURI = 'https://rra.ram.moe/i/r?type=slap',
...
module.exports = class Slap extends Command {
    constructor(bot) {
        super(bot, {
            name: 'slap',
            dirname: __dirname,
            aliases: ["tapa", "headpat", 'pet'],
            description: 'Slap a user.',
            usage: '<prefix><commandName> [user]',
            botPermissions: [Flags.SendMessages, Flags.EmbedLinks],
            cooldown: 3000,
            examples: ['/slap', '.slap @Hope', '!tapa @Klaus @Hope', '?headpat @Klaus @Hope @Faith'],
            slash: true,
            options: [{
                name: 'user',
    ...
    async run(bot, message, settings) {
        ...
        detectUserfromArgs(message).then(mentions => {
            ...
            fetch(baseURI).then(res => res.json()).then(json => {
                ...
                if (mentions.length < 1) {
                    embed.setDescription(message.translate('Actions/slap:TAPA_DESCRIÇÃO', { member: message.member }))
                ...
                } else if (mentions.includes(message.translate('Actions/slap:TAPA_DESCRIÇÃO1'))) {
                    embed.setDescription(`${message.member} ${message.translate('Actions/slap:TAPA_DESCRIÇÃO2')} ${stringifyMentions(mentions)}!`)
                    return message.channel.send({ embeds: [embed] }).then(m => m.timedDelete({ timeout: 15000 })).then(() => {
                        message.channel.send(message.translate('Actions/slap:TAPA_HopeFRASE')).then(async (m) => { if (settings.ModerationClearReplyToggle === true) { m.timedDelete({ timeout: 15000 }) } });
                    })
                } else {
                    embed.setDescription(`${message.member} ${message.translate('Actions/slap:TAPA_DESCRIÇÃO2')} ${stringifyMentions(mentions)}!`)
                    return message.channel.send({ embeds: [embed] }).then(async (m) => { if (settings.ModerationClearReplyToggle === true) { m.timedDelete({ timeout: 60000 }) } });
        ...
        function stringifyMentions(output) {
            if (output.length === 2) {
                output = output.join(message.translate('Actions/slap:TAPA_DESC'))
            } else if (output.length > 2) {
                let last = output.pop()
                output = output.join(', ') + `${message.translate('Actions/slap:TAPA_DESC1')}` + last
            }
            return (output)
        }
            ...
            return new Promise((resolve, reject) => {
                ...
                members.forEach(member => {
                    ...
                    if (member.id === message.member.id) {
                        ...
                        output.push(message.translate('Actions/slap:TAPA_DESC2'))
                    } else
    ...
    async callback(bot, interaction, guild, args) {
        ...
        try {
            ...
            fetch(baseURI).then(res => res.json()).then(json => {
                embed.setImage(uri + json.path)
                if (mentions.length < 1) {
                    embed.setDescription(guild.translate('Actions/slap:TAPA_DESCRIÇÃO', { member: member }))
                    embed.setImage(selfSlap())
                    // console.log(selfSlap())
                ...
                } else if (mentions.includes(guild.translate('Actions/slap:TAPA_DESCRIÇÃO1'))) {
                    embed.setDescription(`${member} ${guild.translate('Actions/slap:TAPA_DESCRIÇÃO2')} ${stringifyMentions(mentions)}!`)
                    return interaction.editReply({ embeds: [embed] }).then(m => { if (guild.settings.ModerationClearReplyToggle === true) { m.timedDelete({ timeout: 60000 }) } }).then(() => {
                        interaction.editReply({ content: guild.translate('Actions/slap:TAPA_HopeFRASE') }).then(async (m) => { if (guild.settings.ModerationClearReplyToggle === true) { m.timedDelete({ timeout: 15000 }) } });
                    })
                } else {
                    embed.setDescription(`${member} ${guild.translate('Actions/slap:TAPA_DESCRIÇÃO2')} ${stringifyMentions(mentions)}!`)
                    return interaction.editReply({ embeds: [embed] }).then(async (m) => { if (guild.settings.ModerationClearReplyToggle === true) { m.timedDelete({ timeout: 60000 }) } });
