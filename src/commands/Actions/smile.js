...
module.exports = class Smile extends Command {
    constructor(bot) {
        super(bot, {
            ...
            dirname: __dirname,
            aliases: ['sorrir'],
            description: 'Smile for a user.',
            usage: '<prefix><commandName> [user]',
            botPermissions: [Flags.SendMessages, Flags.EmbedLinks],
            cooldown: 3000,
            examples: ['/smile', '.sorrir @Klaus'],
            slash: true,
            options: [{
                name: 'user',
    ...
    async run(bot, message, settings) {
        ...
        const url = `https://i.imgur.com/${actions.smile[Math.floor(Math.random() * (actions.smile.length - 1))]}.gif`;
        ...
        if ((message.guild && !message.mentions.members.size) || !message.args[0]) {
            embed.setDescription(message.translate('Actions/smile:SORRIR_DESCRIÇÃO', { author: message.author }))
            ...
            embed.setDescription(message.translate('Actions/smile:SORRIR_DESCRIÇÃO2'))
            return message.channel.send({ embeds: [embed] }).then(async (m) => { if (settings.ModerationClearReplyToggle === true) { m.timedDelete({ timeout: 60000 }) } });

        } else if (new RegExp(`<@!?${message.author.id}>`).test(message.args[0])) {
            embed.setDescription(message.translate('Actions/smile:SORRIR_DESCRIÇÃO1', { author: message.author }))
            return message.channel.send({ embeds: [embed] }).then(async (m) => { if (settings.ModerationClearReplyToggle === true) { m.timedDelete({ timeout: 60000 }) } });

        } else {
            embed.setDescription(`${message.author} ${message.translate('Actions/smile:SORRIR_DESCRIÇÃO3')} ${message.args[0]}!`)
            return message.channel.send({ embeds: [embed] }).then(async (m) => { if (settings.ModerationClearReplyToggle === true) { m.timedDelete({ timeout: 60000 }) } });
    ...
    async callback(bot, interaction, guild, args) {
        ...
        try {
            ...
            if (!args.get('user')) {
                embed.setDescription(guild.translate('Actions/smile:SORRIR_DESCRIÇÃO', { author: interaction.user }))
                ...
                embed.setDescription(guild.translate('Actions/smile:SORRIR_DESCRIÇÃO2'))
                return interaction.editReply({ embeds: [embed] }).then(async (m) => { if (guild.settings.ModerationClearReplyToggle === true) { m.timedDelete({ timeout: 60000 }) } });

            } else if (new RegExp(`<@!?${interaction.user.id}>`).test(member)) {
                embed.setDescription(guild.translate('Actions/smile:SORRIR_DESCRIÇÃO1', { author: interaction.user }))
                return interaction.editReply({ embeds: [embed] }).then(async (m) => { if (guild.settings.ModerationClearReplyToggle === true) { m.timedDelete({ timeout: 60000 }) } });

            } else {
                embed.setDescription(`${interaction.user} ${guild.translate('Actions/smile:SORRIR_DESCRIÇÃO3')} ${member}!`)
                return interaction.editReply(({ embeds: [embed] })).then(async (m) => { if (guild.settings.ModerationClearReplyToggle === true) { m.timedDelete({ timeout: 60000 }) } });
            };
        } catch (error) {
