...
module.exports = class Poke extends Command {
    constructor(bot) {
        super(bot, {
            ...
            cooldown: 3000,
            examples: ['/poke @Klaus', '.cutucar @Hope'],
            slash: true,
            options: [{
                name: 'user',
    ...
    async run(bot, message, settings) {
        ...
        const url = `https://i.imgur.com/${actions.poke[Math.floor(Math.random() * (actions.poke.length - 1))]}.gif`;
        ...
        if ((message.guild && !message.mentions.members.size) || !message.args[0]) {
            ...
            return message.channel.send(message.translate('Actions/poke:CUTUCAR_DESCRIÇÃO', { author: message.author })).then(m => m.timedDelete({ timeout: 10000 }));

        } else if (new RegExp(`<@!?${bot.user.id}>`).test(message.args[0])) {
            embed.setDescription(message.translate('Actions/poke:CUTUCAR_DESCRIÇÃO1'))
            ...
            return message.channel.send(message.translate('Actions/poke:CUTUCAR_DESCRIÇÃO2', { author: message.author })).then(m => m.timedDelete({ timeout: 10000 }));

        } else {
            embed.setDescription(`${message.member} ${message.translate('Actions/poke:CUTUCAR_DESCRIÇÃO3')} ${message.args[0]}`)
            return message.channel.send(
                ({ embeds: [embed] })
    ...
    async callback(bot, interaction, guild, args) {
        ...
        try {
            ...
            if (!args.get('user')) {
                return interaction.editReply(guild.translate('Actions/poke:CUTUCAR_DESCRIÇÃO', { author: interaction.user })).then(async (m) => { if (guild.settings.ModerationClearReplyToggle === true) { m.timedDelete({ timeout: 15000 }) } });
            } else if (new RegExp(`<@!?${bot.user.id}>`).test(member)) {
                embed.setDescription(guild.translate('Actions/poke:CUTUCAR_DESCRIÇÃO1'))
                ...
                return interaction.editReply(guild.translate('Actions/poke:CUTUCAR_DESCRIÇÃO2', { author: interaction.user })).then(m => m.timedDelete({ timeout: 10000 }));
            } else {
                embed.setDescription(`${interaction.user} ${guild.translate('Actions/poke:CUTUCAR_DESCRIÇÃO3')} ${member}`)
