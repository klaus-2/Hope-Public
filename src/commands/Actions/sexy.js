...
const { Embed } = require(`../../utils`),
    ...
    baseURI = 'https://rra.ram.moe/i/r?type=lewd',
    uri = 'https://rra.ram.moe',
    Command = require('../../structures/Command.js');

module.exports = class Sexy extends Command {
    constructor(bot) {
        super(bot, {
            name: 'sexy',
            aliases: ['lewd'],
            ...
            cooldown: 3000,
            examples: ['/sexy', '.sexy @Hope', '!sexy @Hope @Klaus'],
            slash: true,
            options: [{
                name: 'user',
    ...
    async run(bot, message, settings) {
        ...
        detectUserfromArgs(message).then(mentions => {
            ...
            fetch(baseURI).then(res => res.json()).then(json => {
                ...
                if (mentions.length < 1) {
                    embed.setDescription(message.translate('Actions/sexy:SEXY_DESCRIÇÃO', { member: message.member }));
                ...
                } else if (mentions.includes(message.translate('Actions/sexy:SEXY_DESCRIÇÃO1'))) {
                    embed.setDescription(`${message.member} ${message.translate('Actions/sexy:SEXY_DESCRIÇÃO2')} ${stringifyMentions(mentions)} ${(mentions.length === 1) ? message.translate('Actions/sexy:SEXY_EXTRA') : message.translate('Actions/sexy:SEXY_EXTRA1')} ${message.translate('Actions/sexy:SEXY_EXTRA4')}`)
                    return message.channel.send({ embeds: [embed] }).then(m => m.timedDelete({ timeout: 15000 })).then(() => {
                        message.channel.send(message.translate('Actions/sexy:SEXY_DESCRIÇÃO3')).then(async (m) => { if (settings.ModerationClearReplyToggle === true) { m.timedDelete({ timeout: 15000 }) } });
                    })
                } else {
                    embed.setDescription(`${message.member} ${message.translate('Actions/sexy:SEXY_DESCRIÇÃO2')} ${stringifyMentions(mentions)} ${(mentions.length === 1) ? message.translate('Actions/sexy:SEXY_EXTRA2') : message.translate('Actions/sexy:SEXY_EXTRA3')} ${message.translate('Actions/sexy:SEXY_EXTRA4')}`)
                    return message.channel.send({ embeds: [embed] }).then(async (m) => { if (settings.ModerationClearReplyToggle === true) { m.timedDelete({ timeout: 60000 }) } });
                }
            })
        })
        function stringifyMentions(output) {
            if (output.length === 2) {
                output = output.join(message.translate('Actions/sexy:SEXY_DESC'))
            } else if (output.length > 2) {
                let last = output.pop()
                output = output.join(', ') + `${message.translate('Actions/sexy:SEXY_DESC1')}` + last
            }
            return (output)
        }
            ...
            return new Promise((resolve, reject) => {
                ...
                members.forEach(member => {
                    ...
                    if (member.id === message.member.id) {
                        ...
                        output.push(message.translate('Actions/sexy:SEXY_DESCRIÇÃO1'))
                    } else
    ...
    async callback(bot, interaction, guild, args) {
        ...
        try {
            ...
            fetch(baseURI).then(res => res.json()).then(json => {
                embed.setImage(uri + json.path)
                if (mentions.length < 1) {
                    embed.setDescription(guild.translate('Actions/sexy:SEXY_DESCRIÇÃO', { member: member }));
                ...
                } else if (mentions.includes(guild.translate('Actions/sexy:SEXY_DESCRIÇÃO1'))) {
                    embed.setDescription(`${member} ${guild.translate('Actions/sexy:SEXY_DESCRIÇÃO2')} ${stringifyMentionss(mentions)} ${(mentions.length === 1) ? guild.translate('Actions/sexy:SEXY_EXTRA') : guild.translate('Actions/sexy:SEXY_EXTRA1')} ${guild.translate('Actions/sexy:SEXY_EXTRA4')}`)
                    return interaction.editReply({ embeds: [embed] }).then(m => m.timedDelete({ timeout: 15000 })).then(() => {
                        interaction.editReply({ content: guild.translate('Actions/sexy:SEXY_DESCRIÇÃO3') }).then(async (m) => { if (guild.settings.ModerationClearReplyToggle === true) { m.timedDelete({ timeout: 15000 }) } });
                    })
                } else {
                    embed.setDescription(`${member} ${guild.translate('Actions/sexy:SEXY_DESCRIÇÃO2')} ${stringifyMentionss(mentions)} ${(mentions.length === 1) ? guild.translate('Actions/sexy:SEXY_EXTRA2') : guild.translate('Actions/sexy:SEXY_EXTRA3')} ${guild.translate('Actions/sexy:SEXY_EXTRA4')}`)
                    return interaction.editReply({ embeds: [embed] }).then(async (m) => { if (guild.settings.ModerationClearReplyToggle === true) { m.timedDelete({ timeout: 60000 }) } });
                }
            });
        } catch (error) {
