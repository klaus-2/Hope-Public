const { Embed } = require(`../utils`);

module.exports = class Paginate{
  constructor(...array){

    // Array of Embed instance to paginate
    this._array = [ ...array ].flat();

    // the current index of this Pagination
    this._index = 0;

    // Validate array content
    this._validate();
  };

  // Add more Embed to the array
  add(...item){
    this._array.push(...item.flat());
    this._validate()
    return [ ...item.flat() ];
  };

  // Delete some elements from the array
  delete(index){
    if (typeof index !== 'number'){
      return [];
    } else {
      if (index === this.currentIndex){
        if (this.currentIndex > 0){
          this.previous();
        };
      } else if (this.currentIndex === this.tail){
        this.previous();
      };
      return this._array.splice(index,1);
    };
  };

  // Moves the index up to view the next element from the array (Circular - will revert to 0 if the index exceeds array length)
  next(){
    if (!this._array.length){
      return undefined;
    };
    if (this._index === this.tail) this._index = -1;
    this._index++;
    return this._array[this._index];
  }

  // Moves the index down to view the previous element from the array (Circular - will revert to the max index if the index is less than 0)
  previous(){
    if (!this._array.length){
      return undefined;
    };
    if (this._index === 0) this._index = this.tail + 1;
    this._index--;
    return this._array[this._index];
  }

  // The current embed using the current index
  get currentPage(){
    return this._array[this._index];
  }

  // The first embed from the array
  get firstPage(){
    return this._array[0];
  }

  // The last embed from the array
  get lastPage(){
    return this._array[this.tail];
  }


  // The current index
  get currentIndex(){
    return this._index
  }

  // The number of embed in the array
  get size(){
    return this._array.length;
  };

  // The last index, or null if no element.
  get tail(){
    return this._array.length > 0
    ? this._array.length - 1
    : null;
  };

  // Checks if there is a non message embed present in the array
  _validate(){
    for (const el of this._array){
      if (!(el instanceof Embed)){
        throw new Error('Paginate: Passed argument is not an instance of Embed!')
      };
    };
    return;
  };
};
