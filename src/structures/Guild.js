...
module.exports = Object.defineProperties(Guild.prototype, {
	...
	fetchSettings: {
		value: async function() {
			this.settings = await GuildSchema.findOne({ guildID: this.id }) ?? require(`${process.cwd()}/assets/json/defaultGuildSettings.json`);
	...
	updateGuild: {
		value: async function(settings) {
			...
			if (this.settings.guildID) {
				...
				const newGuild = new GuildSchema(Object.assign({ guildID: this.id, guildName: this.name, updatesChannel: this.systemChannel.id }, settings));
				await newGuild.save();
			}
