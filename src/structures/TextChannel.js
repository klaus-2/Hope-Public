		...
		value: function (key, args, isForEmbed) {
			try {
				...
				const embed = new EmbedBuilder()
					.setColor(16741245)
					.setDescription(`<:cancel:855859391709577266> ${this.client.translate(key, args, this.guild.settings.Language) ?? key}`);
	...
	success: {
		value: function (key, args, isForEmbed) {
			try {
				...
				const embed = new EmbedBuilder()
					.setColor(10147968)
					.setDescription(`<:v_:855859391835799582> ${this.client.translate(key, args, this.guild.settings.Language) ?? key}`);
