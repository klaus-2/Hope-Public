module.exports = async function (channel, message, options) {
  const init = new Promise(async resolve => { // Create Promise
    ...
    // Assign Webhook
    let hook = webhooks.find(w => w.name === 'https://hopebot.top/')
    if (!hook) {
        try {
            hook = await channel.createWebhook('https://hopebot.top/', {
                avatar: `https://hopebot.top/logo.png`
            });
        } catch (e) {
            hook = await channel.createWebhook('https://hopebot.top', `https://hopebot.top/logo.png`);
        }
