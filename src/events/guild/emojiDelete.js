...
module.exports = class emojiDelete extends Event {
	...
	async run(bot, emoji) {
		...
		// For debugging
		if (bot.config.debug) bot.logger.debug(`Emoji: ${emoji.name} foi deletado do servidor: ${emoji.guild.name} [${emoji.guild.id}].`);
		...
		if (logDB.ServerEvents.EmojiUpdateToggle == true && logDB.ServerEvents.Toggle == true) {
			...
			const embed = new Embed(bot, emoji.guild)
				.setDescription(`<:v_:855859391835799582> **${emoji.guild.translate('Events/emojiDelete:EMOJI_DELETE')} ${emoji} (${emoji.name}) ${emoji.guild.translate('Events/emojiDelete:EMOJI_DELETE1')}**`)
				.setColor(color)
				.setFooter({ text: `ID: ${emoji.id}` })
				.setAuthor({ name: `${emoji.guild.translate('Events/emojiDelete:EMOJI_DELETE2')}`, iconURL: `https://media.giphy.com/media/3kIkf9hTaie4ieG9D0/giphy.gif` })
				.setTimestamp();
