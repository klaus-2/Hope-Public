...
module.exports = class roleCreate extends Event {
	...
	async run(bot, role) {
		...
		// For debugging
		if (bot.config.debug) bot.logger.debug(`O Cargo: ${role.name} foi criado no servidor: ${role.guild.name} [${role.guild.id}].`);
		...
		if (logDB.ServerEvents.RoleCreateToggle == true && logDB.ServerEvents.Toggle == true) {
			...
			const embed = new Embed(bot, role.guild)
				.setDescription(`${role.guild.translate('Events/roleCreate:ROLE_CREATE')} **${role.name}**\n ${role.guild.translate('Events/roleCreate:ROLE_CREATE1')} **${role}**`)
				.setColor(color)
				.setFooter({ text: `ID: ${role.id}` })
				.setAuthor({ name: `${role.guild.translate('Events/roleCreate:ROLE_CREATE2')}`, iconURL: `https://media.giphy.com/media/3kIkf9hTaie4ieG9D0/giphy.gif` })
				.setTimestamp();
