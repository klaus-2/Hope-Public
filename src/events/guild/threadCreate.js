...
module.exports = class threadCreate extends Event {
	...
	async run(bot, thread) {
		...
		// For debugging
		if (bot.config.debug) bot.logger.debug(`Thread: ${thread.name} has been created in guild: ${thread.guildId}. (${inf[thread.type]})`);
		...
		if (logDB.ServerEvents.ThreadCreateToggle == true && logDB.ServerEvents.Toggle == true) {
			...
			const embed = new Embed(bot, thread.guild)
				.setDescription([
					`**${inf[thread.type]} created: ${thread.toString()}**`,
					'',
